...
app.get("/photosOfUser/:id", async function (request, response) {
  ...
  try {
    ...
    if (photos.length === 0) {
      response
        .status(200)
        .json({ photos: [], message: "No photos found for the user" });
    } else {
      ...
      response.status(200).json({photos: photos, message: ""});
    }
  } catch (error) {
...
app.post("/admin/login", async function (request, response) {
  ...
  if (!user || user.password !== password) {
    ...
    // console.log("SAVING SESSION: ", request.session);
...
app.delete(
  "/photosOfuser/:photo_id/:comment_id",
  async function (request, response) {
    ...
    try {
      ...
      photo.comments.forEach((comment) => {
        if (String(comment._id) == commentId) commentOwnerID = comment.user_id;
      });
      // Check if the user owns the comment (you can modify this check based on your authentication and ownership logic)
      // console.log("SESSION: ", request.session);
      // console.log("USER COMPARE: ", userId, commentOwnerID);
      if (String(userId) !== String(commentOwnerID)) {
        response
          .status(401)
          .json({ error: "Unauthorized - You don't own this comment" });
        return;
      }
      ...
      response
        .status(200)
        .json({ photo, message: "Comment deleted successfully" });
    } catch (error) {
      response.status(500).json({ error: error.message });
    }
  }
...
app.delete("/photosOfuser/:photo_id", async function (request, response) {
  const userId = request.session.user ? request.session.user._id : null;
  const photoId = request.params.photo_id;
  ...
  try {
    ...
    if (userId !== String(photo.user_id)) {
      response
        .status(401)
        .json({ error: "Unauthorized - You don't own this photo" });
      return;
    }
...
app.delete("/user/:id", async function (request, response) {
  ...
  photos.forEach(async (photo) => {
    //delete any photos that the user owns
    if (String(userId) === String(photo.user_id)) {
      fs.unlinkSync('./images/' + photo.file_name);
      photo.delete();
      return;
    }
    //delete any comments the user made
    photo.comments = photo.comments.filter((comment) => {
      return String(userId) !== String(comment.user_id);
    });
    //delete any likes the user has left
    photo.likes = photo.likes.filter((likeOwnerId) => {
      return String(userId) !== String(likeOwnerId);
    });
    await photo.save();
  });
...
app.post("/photos/new", loggedInCheck, function (request, response) {
  ...
  app.post("/photos/like/:photoId", async function (req, res) {
    ...
    try {
      ...
      const likedPhotos = (req.session.user && req.session.user.likes) || [];
  
      res.json({
        message: "Like status updated successfully",
        photos: {
          likes: photo.likes,
          liked: likedPhotos.includes(photoId), 
        },
      });
    } catch (error) {
  ...
  app.use(["/photos/like/:photoId"], loggedInCheck);
